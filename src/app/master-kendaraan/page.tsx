...
export default function Karyawan() {
  ...
  const { data, error } = useSWR(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/kendaraan?limit=${filter.limit}&sort_by=${filter.sort_by}&page=${filter.page}`,
    fetcher
  );
  useEffect(() => {
    if (data !== undefined) {
      ...
      <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
        <div className="flex">
          <div className="flex gap-2">
            ...
            >
              ...
              <span className="sr-only sm:not-sr-only">Kendaraan</span>
            </Button>
        ...
        <Card className="w-full">
          <CardHeader>
            ...
            <CardDescription>
              Data Kendaraan dikelola dan informasi lengkap dapat diperoleh di
              sini.
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <TableBody>
                {data.data.data.map((item: any, index: number) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>{item.kode_kendaraan}</TableCell>
                    <TableCell className="font-medium">
                      {item.nama_kendaraan}
                    </TableCell>
                    <TableCell>{item.jenis_kendaraan}</TableCell>
                    <TableCell>{item.merk}</TableCell>
                    <TableCell>{item.warna}</TableCell>
                    <TableCell>{item.nomor_polisi}</TableCell>
                    <TableCell>{item.nomor_mesin}</TableCell>
                    <TableCell>{item.nomor_chasis}</TableCell>
                    <TableCell>
                      <DropdownMenu>
